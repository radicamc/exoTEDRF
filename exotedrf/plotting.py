...
def plot_average_covariance(matrices, labels=None, xspan=None):
    """Collapse a covariance matrix along the diagonal to show the average covariance.
    """
    ...
    for j, matrix in enumerate(matrices):
        ...
        for i in range(nbins):
            ...
            stack[i, ax] = matrix[:, i]
            plt.plot(np.arange(nbins) - i, matrix[:, i], c=colours2[j], lw=0.1, alpha=0.5)
        plt.plot(np.arange(2 * nbins) - nbins, np.nanmean(stack, axis=0), c=colours[j],
                 label=labels[j], zorder=1000)
    plt.axhline(0, ls='--', c='grey')
